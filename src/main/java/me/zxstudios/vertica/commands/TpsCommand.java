...
public class TpsCommand implements CommandExecutor {
    ...
    public TpsCommand(Vertica plugin) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (!(player.hasPermission(config.get("tps.permission")))) {
        ...
        player.sendActionBar(config.get("tps.message").replace("%tps%", String.valueOf(Bukkit.getTPS()[0])));
        soundLib.play(player, "tps.message-sound");
